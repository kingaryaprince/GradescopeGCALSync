...
**Palette is teal**, derived from `#2c9caf`/`#70bdca`; it sits closer to Gradescope's own dark
teal than the original indigo did. Red/amber stay reserved for urgency, which is also why
`lib/colors.ts` excludes hues 0-20 from course chips.

**Course chips** come from `lib/colors.ts`: a curated hue ring indexed by a hash of the course
id, not the raw hash (which drifts into muddy yellows and can collide with urgency red).
...
**Inter is self-hosted** (`src/styles/inter-latin.woff2`, weight-axis latin subset, 48KB). Do
not swap it for a CDN link: the privacy policy promises no external requests.

Icons are generated by a script in the commit history rather than a design tool, and the 16px
variant uses **simplified geometry** (bigger body, thicker header, larger date mark) because
the detailed one turns to mush at toolbar size.
